...
class OrderTest
  ...
  "Filter orders in the interval should" - {
    "return Some(List[Order]) if not empty" in {
      MainService.ordersInTheInterval(ordersList, date1LDT, date4LDT) should be(Some(List(order1)))

    }
    "return None if empty List" in {
      MainService.ordersInTheInterval(ordersList, date3LDT, date4LDT) should be(None)
    }
  }
}
